...
## Session 1: Command Parser & Registry ✅ Done
...
## Session 3: Virtual Filesystem ✅ Done
...
## Session 4: Filesystem Commands ✅ Done
...
## Session 6: Network Simulation ✅ Done
...
## Session 7: Network Commands ✅ Done
...
## Session 8: Heat System ✅ Done

> Build the heat/trace system. Read `CLAUDE.md` and `docs/specs/05_heat_system.md`. Implement `hackerzork/systems/heat.py` with the `HeatSystem` class. It listens to events from the event bus and accumulates heat. Implement thresholds (safe, monitored, active_response, hunted, critical, burned), decay over time, stealth modifiers, and the identity burn mechanic at 100. Emit `heat_changed` and `heat_threshold_crossed` events. Write tests in `tests/test_systems/test_heat.py`. Run pytest when done.

---

## Session 9: Toolkit & Package System ← Next

> Build the toolkit and package management system. Read `CLAUDE.md` and `docs/specs/09_toolkit_unlocks.md`. Implement `hackerzork/systems/toolkit.py` (tracks installed packages, active stealth tools, package state machine, and the "poison" mechanic for shadow packages) and `hackerzork/commands/packaging.py` with commands: `apt` (mainline packages), `shadow` (underground packages), `gpg` (key management). Packages unlock new commands, add stealth modifiers to `HeatSystem`, and some are traps. Create YAML definitions for a few starter packages under `hackerzork/data/packages/apt/` and `hackerzork/data/packages/shadow/`. Write tests in `tests/test_systems/test_toolkit.py` and `tests/test_commands/test_packaging.py`. Run pytest when done.

---

## Session 10: System Commands

> Build the system/utility commands. Read `CLAUDE.md` and look at existing command implementations in `hackerzork/commands/`. Implement `hackerzork/commands/system.py` with: `whoami`, `uname`, `ps`, `top`, `man`, `history`, `clear`, `env`, `export`, `alias`, `echo`, `date`. These give the fake OS its personality. `man` should show help for any registered command from the registry. `ps` and `top` should show fake processes including some suspicious SkyNet-adjacent entries. `uname` returns fake OS info (flavor of "HackerOS"). Write tests in `tests/test_commands/test_system.py`. Run pytest when done.

---

## Session 11: Audio System

> Build the audio system. Read `CLAUDE.md` and `docs/specs/06_audio.md`. Implement `hackerzork/audio/mixer.py` (4-layer audio manager using pygame.mixer), `hackerzork/audio/ambient.py` (drone management), `hackerzork/audio/sfx.py` (sound effect triggers), and `hackerzork/audio/reactive.py` (heat-reactive audio mixing). The system must gracefully degrade when audio is disabled or sound files are missing. Wire the reactive system to listen for `heat_changed` and `heat_threshold_crossed` events from the bus. Write tests — mock pygame.mixer so tests run without audio hardware. Run pytest when done.

---

## Session 12: Visual Effects
...
## Session 16: Integration & Boot Sequence

> Wire everything together and build the cold boot experience. Update `hackerzork/game.py` to initialize all systems in the correct dependency order, wire up event listeners, and load the initial game state. Implement the full boot sequence animation (POST, memory check, services starting, prompt appearing). Make sure `python -m hackerzork.main` boots into a fully playable shell with the home filesystem, working commands, audio, and heat tracking active. Create the help command in `hackerzork/commands/help.py`. Run the full test suite. Run pytest when done.
