...
│   │   └── censored.yaml    # SkyNet's redacted vault (74 satire files)  ✅ DONE
...
- Censored flag: `FSEntry.censored` + `censor_quip` + `censor_heat` — `cat`/`head`/`tail`/`grep` print the quip and bump heat instead of leaking content; `rm`/`cp`/`mv` refuse and spike heat by 15. Cat caps total censor-heat at 30 per command (prevents `cat *.enc` glob from cascading through identity_burn). One `censored_file_accessed` event per command, not per file.
...
- Censored vault — political/conspiracy satire content as a gameplay system. New `censored: bool` + `censor_quip: str` + `censor_heat: float` fields on the VFS, all serialized through save/load. When `cat`/`head`/`tail`/`grep` hit a censored file they print SkyNet's refusal block and bump heat by `censor_heat`; when `rm`/`cp`/`mv` hit one they refuse with "SkyNet has flagged this file for retention" and spike heat by 15 (the `--shred` flag is the story-level escape). Cat caps total censor-heat per command at 30 — without the cap, `cat /var/skynet/censored/*.enc` over the 60+ vault files cascaded heat through 100 → identity_burn → 0 → 100 → identity_burn TWICE in one keystroke and instantly maxed SkyNet awareness. Same fix: cat emits ONE `censored_file_accessed` event per command (not per file). Content lives in `data/filesystem/censored.yaml` (74 entries, ~50 in the public vault `/var/skynet/censored/`, 3 in `PRISM_intercepts/`, ~10 plaintext files in the hidden `/var/skynet/.private/` embarrassment folder, plus a Z0RK-7 README at `/home/user/notes/leaked_drops/`). Tone discipline applied: satire is pointed at censorship, bureaucracy, and political cowardice — never at victims, minors, or crimes; conspiracies are mocked by deflation (empty files, anti-climax payoffs); the Hunter Biden laptop file's "decryption" payload is a Rick Astley link. Two boot service lines added to `effects/animations.py` (`sk_altman_appeasement` OK, `sk_smoking_gun_extractor` FAILED attempt 14287); 3 new tier-3 SkyNet barks + 4 new tier-5 `_ADDRESS_MESSAGES` covering the censored-vault world. Shared censor helpers (`_censor_block`, `_accumulate_censor_heat`, `_emit_censored_seen`, `_refuse_censored_tamper`) so cat/head/tail/grep/rm/cp/mv all flow through the same code paths. Also fixed a pre-existing WS bug surfaced during this work: `_queue_heat_event` was reading field `heat` from heat_changed events, but heat.py emits with field `level` — so the browser HUD has been displaying `0.0` on every heat change since the WS layer was first written. 31 new vault tests + 1 WS regression test, **1899 passing**.
